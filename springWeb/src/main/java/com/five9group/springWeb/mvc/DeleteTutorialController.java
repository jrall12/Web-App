...
public class DeleteTutorialController {
	...
	static Logger log = Logger.getLogger(DeleteTutorialController.class.getName());
	
	@Autowired
	TutorialDAO tutorailDAO;
	
	@Autowired
    @Qualifier("DeleteTutorialValidator")
	...
	public ModelAndView showDeleteTutorial() {
		return new ModelAndView("deleteTutorial", "command", new Tutorial());

	}


	@RequestMapping(value = "/deleteTutorial", method = RequestMethod.POST)
	public String deleteTutorial(@ModelAttribute("tutorial")  @Validated Tutorial tutorial, BindingResult result) {

		 if(result.hasErrors()){
			 log.info("Returning deleteTutorial.jsp page");
	            return "deleteTutorial";

		 }
		try {
		    log.info("Starting to Delete tutorial from database");
			tutorailDAO.deleteTutorial(tutorial);
